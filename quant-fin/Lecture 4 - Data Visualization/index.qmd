...
FANG%>%write.csv('Assets/FANG.csv',row.names = FALSE)
...
-   Part of your work as a Financial Analyst is to *convey* the information to a broader public:
    1.  Fund managers do not use *R*, *Python*, and won't be proof-reading your *.xlsx* spreadsheet
...
-   It is imperative to think about *how* you should structure your message to effectively communicate it to your audience's needs. I introduce you the *Grammar of Graphics*:
...
-   How can you implement these foundations to showcase your data analysis? Luckily, there is a way to do that in `R`
...
-   We will illustrate the use of `ggplot2` to replicate the *Grammar of Graphics* foundations using the `FANG` dataset, which is loaded together with your slides - if you prefer to do it direclty in `R`, hit the download button and load it using `read.csv('FANG.csv')`
...
-   In what follows, we will be working with a step-by-step example of how to use `ggplot2` for data visualizations. It is important that you interactively
...
-   To check all available `themes`, check the [`ggthemes`](https://yutannihilation.github.io/allYourFigureAreBelongToUs/ggthemes/) website
