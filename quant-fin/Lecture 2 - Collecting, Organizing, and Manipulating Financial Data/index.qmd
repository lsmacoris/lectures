...
# Package names
packages <- c("tidyverse","tidyquant","tidymodels","xts","glue","scales","downloadthis")
...
#List of assets
assets= c('AAPL','MSFT','NVDA','GOOG','AMZN','META','TSLA','F','MMM','GME')
start='2020-01-01'

#Write to memory
Data=tq_get(assets,from=start,to=Sys.Date())
Data%>%write.csv('Assets/M7.csv',row.names = FALSE)
...
-   As part of your work as a buy-side analyst, you were asked to analyze how the *Magnificent 7* performed after the *DeepSeek* episode. Follow the instructions below and answer to the following question: **Which stock suffered the most up to yesterday?**

::: callout-tip
### Exercise

1.  Select only the tickers that refer to the *Magnificent 7* group
2.  Filter only on observations happening in 2025
3.  Arrange the dataset (from oldest to newest)
4.  Create, for each different `symbol`, a `Return` variable that is defined as $P_{t+1}/P_{t}$, where $t$ refers to a date
5.  Create, for each different `symbol`, a `Cum_Return` return variable that is defined as the cumulative return
6.  Pick the latest observation from each `symbol` and arrange the table from lowest-to-highest return

-   The data, stored in `M7.csv`, can be loaded using `read.csv('M7.csv')`
-   You can use the `year()` function to retrieve the year of a given `Date` column. Also, `slice_tail(n=x)` retrieves the bottom `x` observations, whereas `slice_head(n=y)` retrieves the top `y`
:::
