...
## Moving past relative valuation
...
- As you saw in the previous lecture, although *Relative Valuation* can provide a ballpark estimate of what would have been the firm value *as-if* it was priced similarly to other comparable firms, there were a series of drawbacks and limitations:
  ...
  1. Sensitive to market mood and choice of comparables
  2. Captures market sentiment and industry positioning
  3. Generally backward-looking
...
Valuation method that estimates the present value of an asset based on its [expected future cash flows.]{.blue}. It takes into account measures of expected profits ($\small 1,2,...,\infty$ periods ahead) and its underlying risk to come up with an estimate of the opportunity value in $\small t_0$. 
:::

1. [Core Idea]{.blue}: if we are able to estimate the firms' future cash flows, we can adjust them to present value using a discount rate
2. [Key Inputs]{.blue}: Cash flow projections, discount rate (often WACC), and terminal value assumptions
3. [Strengths]{.blue}: Grounded in fundamentals; flexible to different scenarios
4. [Weaknesses]{.blue}: time-consuming, fully dependend on the forecast assumptions
...
3.  Calculating *EBIT*, *Taxes*, and *NOPAT*
