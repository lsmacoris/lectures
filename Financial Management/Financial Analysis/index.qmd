...
-   Investors often use [accounting statements]{.blue} to evaluate a firm in one of two ways:
    ...
    2.  Compare the firm to other similar firms using a common set of [financial ratios]{.blue}.
...
-   To evaluate the mix of debt and equity in a firm's capital structure, we can use:
    ...
    1.  [Liabilities-to-Equity]{.blue}: measures the **overall** solvency:
...
2.  [Debt-to-Equity]{.blue}: focus on the Debt component, which is generally more binding:
...
-   The [Return on Equity (ROE)]{.blue} measures exactly this:
...
-   [ROE]{.blue} is a comprehensive indicator of a firm's performance because it provides an indication of how well managers are employing the funds invested by the firm's shareholders to generate returns:
...
-   **In the long-run, firms are expected to earn a ROE that is higher than its cost of capital:**
    ...
    2.  $\uparrow$ Competition $\rightarrow$ ROE tends to go down to a "normal", benchmark level

-   **Short-term deviations from such "benchmark" level of ROE can occur due to:**

    1.  Industry conditions and competitive strategy
...
-   Say that you want to understand *how* the firm has generated the $30\%$ [ROE]{.blue} previously shown. A firm's ROE can be broke down in two factors:
    1.  The profitably it employs its assets
...
Plot=ggplot(Data,aes(x=Leverage,y=Value,color=Key))+geom_line(size=1)+
theme_minimal()+
scale_colour_manual(values=c('Positive'='darkgreen','Negative'='red'))+
scale_y_continuous(labels = scales::percent)+
scale_x_continuous(breaks=seq(1,2,0.1))+
labs(title='How different Leverage levels affects ROE?',
     subtitle='A simulation using ROA=20% and -20%, varying on Leverage levels',
     x='Leverage',
     y='Return on Equity',
     color='Return on Assets')+
geom_vline(xintercept = 1,linetype='dashed')+
  annotate("text",x=1.25,y=-0.3,label='ROA = -20%',size=10)+
  annotate("text",x=1.25,y=0.3,label='ROA = +20%',size=10)+
...
-   Our ROE decompositon yielded the following results:
    ...
    1.  The Return on Assets is 20%
    2.  Firm's leverage is 1.5
...
$$ Therefore, we can view ROE as a composite of:
...
-   The [Net Profit Margin]{.blue} indicates how much a firm keeps for each dollar of sales it makes
-   The [Asset Turnover]{.blue} indicates how many sales it gets for each dollar of assets
...
2.  Understanding and comparing these specific drivers to benchmarks help us understanding the areas where the managers are over(under)performing
...
-   After figuring out that the amount of assets to be liquidated is around \$164,318, the CFO was given two options:
    ...
    1.  A mix of selling Properly, Plant and Equipment, worth \$120,000 in book value, and liquidating \$44,318 in inventories, to be paid as dividends
    2.  Liquidating \$130,000 in short-term financial investments and \$34,318 in cash, to be paid as dividends
...
c.  Suppose a firm's total assets turnover ratio falls from 1.0 to 0.9, but at the same time its profit margin rises from 9% to 10%, and its debt increases from 40% of total assets to 60%. Under these conditions, the ROE will decrease.
d.  Suppose a firm's total assets turnover ratio falls from 1.0 to 0.9, but at the same time its profit margin rises from 9% to 10% and its debt increases from 40% of total assets to 60%. Under these conditions, the ROE will increase.
...
1.  If Asset Turnover went from 1 to 0.9 $\rightarrow$ a decrease of -10% ($\Delta A.T$)
2.  If Margins went up from 9% to 10% $\rightarrow$ an increase of 11.11% ($\Delta Margin$)
...
-   After deducting *direct* costs, there are still *indirect* operating costs (or revenues) to be taken into consideration. *NOPAT* takes into account all [non-financial]{.blue} components
...
-   NOPAT margin provides a comprehensive indication of the operating performance of a company because it reflects all [operating]{.blue} costs and eliminates the effects of debt policy:
...
-   EBITDA can be useful to contrast operations by disregarding the potential differences stemming from:
...
-   Ideally, the balance sheet would provide us with an accurate assessment of the true value of the firm's equity. Unfortunately, this is unlikely to be the case:
    ...
    1.  Many of the assets listed on the balance sheet are valued based on their historical cost rather than their true value today.
    2.  More importantly, a substantial amount of the firm's valuable assets are not captured on the balance sheet (expertise of its employees, reputation, relationships with customers and suppliers, among others)
...
-   The P/E ratio is a simple measure that is used to assess whether a stock is over- or under-valued based on the idea that the value of a stock should be proportional to the level of earnings it can generate for its shareholders
    ...
    3.  We can avoid this limitation by instead assessing the market value of the underlying business using valuation ratios based on the firm's enterprise value[^1].

[^1]: See BDM Comment on *Mismatched* Ratios, p. 77
...
-   **Question** what could be considered a [sustainable]{.blue} growth rate of a business:
...
*$\rightarrow$ Practice these concepts analyzing [3M]{.blue} using the [Exercise - 3M Ratios.xlsx]{.blue} (available on eClass®)*
...
*$\rightarrow$ Full content on Credit Ratings: see [UV8009 - Credit Rating Basics]{.blue}, available on eClass®*
...
## *Appendix II*: from Financial Ratios to Investment Strategies \[TBD\]
