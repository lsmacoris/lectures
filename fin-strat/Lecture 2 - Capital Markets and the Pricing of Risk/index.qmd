...
format:
  revealjs:
    title: '2. Capital Markets and the Pricing of Risk'
...
::: callout-important
[*Standard Deviation*/*Variance* are correct measures of total risk]{.big}[only if the returns are normally distributed]{.blue}!
:::
...
$$\small 𝑅_{2016}=(0.8499)\times(0.8409)\times(1.0811)\times(0.4404)−1=−65.9\%$$
...
-   Let's use this rationale to understand how increasing the portfolio can reduce its volatility
    ...
    2.  Start from left to right and create a portfolio that consists of 100% of the first stock. Calculate the volatility of the returns. For $n>2$, assign *equal-weights* to each stock
...
$$
\small \dfrac{\Delta \overline R_S}{\Delta \overline{R}_{M}}
$$

-   In [graphical]{.blue} terms: we're trying to find the *slope* of a linear relationship!

-   You are probably seeing that one coming... this is the exact definition of [$\beta$]{.blue} in a linear regression!
...
-   Estimating the [historical]{.blue} Market excess returns is fairly straightforward:
    1.  Define what the [market]{.blue} portfolio is: it can be, for example, a representative index of a countrie's stock exchange, such as the *S&P500* (U.S) or *Ibovespa* (Brazil)
...
$$\small E[R_m] = 60\% \times 15\% + 40\% \times 5\%  = 11\%$$

2.  Second, compute $E[R]$:

. . .

$$\small E[R] = 6\% + 1.18 \times (11\% - 6\%) = 11.9\%$$
