...
-  It corresponds to the diversifiable risk of the stock, which is the risk that is uncorrelated with the market!
...
Returns=do.call('cbind',Prices%>%
                  ...
                  map(na.omit)%>%
                  map(.,~.*100))%>%
  setNames(c('PETR3','IBOV'))%>%as.data.frame()
  
Returns$fitted=lm(PETR3~IBOV,data=Returns)%>%predict()

#Plot
hchart(Returns,type='point',hcaes(x=IBOV,y=PETR3))%>%
hc_add_series(name='Fitted Line',Returns,hcaes(x=IBOV,y=fitted),type = "line")%>%
