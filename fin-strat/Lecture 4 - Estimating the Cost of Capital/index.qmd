...
-   As we'll see throughout the lecture, [CAPM]{.blue} is very practical and straightforward to implement, and the the CAPM-based approach is very robust

    1.  It imposes a disciplined process on managers to identify the cost of capital

    2.  It makes the capital budgeting process less subject to managerial manipulation than if managers could set project costs of capital (*i.e.*, the opportunity cost for the firm's equity) without clear justification

    3.  It is often the model many investors use to evaluate risk

    4.  All in all, it gets managers to think about risk in the correct way: instead of thinking about total risk, the CAPM shows us that we only the market risk (non-diversifiable) should be the concern
...
1.  All investors have access to the same set of securities (*i.e* there is no such investment that is only for *accredited investors*)
2.  All investors are able to buy and sell securities at the same conditions
3.  Finally, they're all able to access a risk-free investment, $R_f$, both for investment and lending
...
1.  Investors that have preferences towards risk will weight in more on the [tangent]{.blue} portfolio and less on the risk-free asset
2.  On the other hand, investors that are risk-adverse will weight in more on the [risk-free]{.blue} asset
...
-   Think about your simplifying assumptions now...
    ...
    3.  Finally, as they can borrow and lend at the same rate $R_f$ (*Assumption #1*), they can adjust the risk according to their preferences
...
2.  What does that mean for us in terms of determining expected equity returns? Recall that, from our previous class, the required return for a given stock $i$ should be:
...
-   The [CAPM]{.blue} implies that there is a *linear* relationship between a stock's sensitivity to the market ($\beta$) and its expected return:
...
-   This relationship has also a name: the [Security Market Line (SML)]{.blue}. It is the line along which all individual securities should lie when plotted according to their expected return and $\beta$
...
$\rightarrow$ *Therefore, the distance of each stock to the right of the capital market line [must]{.blue} be due to its diversifiable risk!*
...
-   First and foremost, what is the definition of a [market portfolio]{.blue}? It is the total supply of securities, with the proportions of each security corresponding to the proportion of the total market that each security represents
...
Returns=do.call('cbind',Prices%>%
                  map(dailyReturn,leading=FALSE)%>%
                  map(na.omit)%>%
                  map(.,~.*100))%>%
  setNames(c('PETR3 returns','IBOV returns'))%>%as.data.frame()
  
Returns$fitted=lm(`PETR3 returns`~`IBOV returns`,data=Returns)%>%predict()

#Plot
hchart(name='Actual Returns', Returns,type='point',hcaes(x=`IBOV returns`,y=`PETR3 returns`))%>%
hc_add_series(name='Expected Returns',Returns,hcaes(x=`IBOV returns`,y=fitted),type = "line")%>%
hc_title(text="PETR3 and IBOV returns (2023-2024)")%>%
hc_subtitle(text='Considering closing adjusted prices from Yahoo! Finance')%>%
hc_tooltip(formatter = JS("function () {
    if (this.series.name === 'Actual Returns') {
      return `<b>${this.series.name}:</b>
              </br><b>Ibovespa:</b> ${this.x.toFixed(2)}%
              </br><b>PETR3:</b> ${this.y.toFixed(2)}%`
      } else if (this.series.name === 'Expected Returns') {
...
-   Furthermore, the decision to invest in specific projects using the firm's cost of capital might ignore the project's risk:
    ...
    3.  Finally, there may be some specific projects, such as *Research and Development (R&D)*, that are likely riskier than the average risk of a project within the company
...
-   Why we should [unlever]{.blue} the return on equity (or, alternatively, the equity $\beta$)? Note that whenever we measure the cost of equity, we are measuring the expected returns based on the [equity]{.blue} risk, which has some implicaitons if a given firm has debt:
...
-   Assuming that the debt $\beta$ is zero, We can also use [X]{.blue}'s unlevered Beta and CAPM:
...
1.  In the first case, we assumed that the expected return debt is equal to its promised yield of 3.2% (which [overstates]{.blue} the return)

2.  In the second, we assumed the debt has a beta of zero, which implies an expected return equal to the risk-free rate of 3% according (which [underestimates]{.blue} the cost of debt

3.  The truth is somewhere between the two results

## Introducing Cash and Net Debt

-   Sometimes, firms maintain large cash balances in excess of their operating needs. Holding cash is a risk-free asset, and as so, [reduces]{.blue} the average risk of the firm's assets

-   Therefore, we should exclude cash holdings when computing the asset's risk, as we're interested in the risk of the firm’s underlying business operations, [separate from its cash position]{.blue}

-   In that case, we can measure the leverage of the firm in terms of its [net debt]{.blue}:
...
-   In mid-2022, [Garmin]{.blue} had a market capitalization of \$18.8 billion, \$100 million in debt, and \$1.6 billion in cash. If its estimated [equity]{.blue} beta was 0.93, estimate the beta of Garmin’s [underlying business]{.blue}

. . .

-   In other words, we want to estimate the beta of Garmin's [assets]{.blue}, which comprise the whole business, not just the equity portion! Note that Garmin's Net Debt is \$ $\small (0.1-1.6)=-1.5$ billion, and the enterprise value is simply $\small \text{Equity + Net Debt = } 18.8-1.6=17.3$. Assuming that cash and debt are both risk-free investments, we can estimate $\beta_U$ as:
...
-   If a firm has more cash than debt, [its net debt will be negative]{.blue}. In this case, its [unlevered]{.blue} beta and cost of capital will exceed its equity beta and cost of capital, as the risk of the firm’s equity is mitigated by its cash holdings!
...
1.  For example, instead of using only one comparable firm to find the unlevered $\beta$, we may use the average (or median) of several firms that are thought of as comparable peers
...
-   We saw that we should evaluate a project’s cost of capital by comparing it with the [unlevered assets of firms in the same line of business]{.blue}

-   However, even firm asset betas reflect the market risk of the [average project]{.blue} in the firm. [Individually]{.blue}, projects can differ in risk:

    1.  For instance, think about a multi-divisional firm, like 3M, where each division will likely have its own level of market risk.
    2.  Even within a firm with a single line of business, some projects obviously have different market risk characteristics from the firm’s other activities, like R&D *versus* product line expansion projects
    3.  Finally, *Operating Leverage* may also play a role - see [@BDM] for a detailed discussion

## Financing with Equity and Debt

-   Thus far, presumed the project we are evaluating is [all-equity]{.blue} financed:

    1.  Whenever we have another comparable firm that is 100% equity-financed and has similar risk, we can use it to estimate expected returns
    2.  If the comparable firm has equity *and* debt, we can *unlever* the cost of capital (or the $\beta$) so as to compare the underlying business

-   What if the firm in which we wish to estimate the cost of capital [is not 100% equity]{.blue} anymore?

-   For cases like this, we will turn our attention to the [Weighted Average Cost of Capital]{.blue} (also known as [WACC]{.blue}), the weighted average of the cost of capital from *all* the firm's claimants:

1.  It takes into account the cost of equity, $r_E$, which is generally estimated using the [CAPM]{.blue}
2.  It also takes into account the fact that debtholders should also expect a return, $r_D$ (which is generally lower than $r_E$) for financing the firm

## The Weighted Average Cost of Capital (WACC)

-   In perfect capital markets (which we'll define later in the course), the [WACC]{.blue} of the firm's assets can be estimated as:
...
-   Because interest expense is often tax deductible (using, for example, a marginal tax-rate $\tau$), the [WACC]{.blue} is *less* than the expected return of the firm's assets - this will make the *"after-tax"* WACC decrease in comparison to the *"pre-tax"* above:
...
-   Note that it is the cost calculated [without]{.blue} any adjustments that could arise due to tax-shiedls

1.  Expected return investors will earn by holding the firm's [assets]{.blue} - *i.e*, the whole business
2.  In a world with taxes, it can be used to evaluate an [all-equity]{.blue} project with the [same risk as the comparable firm]{.blue}

## After-tax WACC

-   The *after-tax WACC* takes into account the fact that debt is generally *tax-deductible*, and therefore should have a [lower]{.blue} effective cost than simply $r_D$. If you assume that a company has a marginal tax-rate of $\tau$, the after-tax WACC is:
...
1.  In a world with taxes, it can be used to evaluate a project with the [same risk and the same financing as the comparable firm]{.blue}
2.  Why? Since the after-tax WACC takes into account the benefitial effects from tax-shields, it can only be applied to cases where *both* the risk *and* the financing structure of the company is equal to the comparable firm!
...
1.  [The Unlevered Cost of Capital ($r_U$)]{.blue} or [pre-tax WACC]{.blue} is:
...
-   In words, if leverage [increases]{.blue}, the equity sensitivity should increase as well
