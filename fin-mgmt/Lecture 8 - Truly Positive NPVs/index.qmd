...
-   In our lecture on *Capital Budgeting*, you saw how the choice of assumptions regarding cash-flows, discount rates, growth rates, among others may severely change NPV estimates

-   When dealing with investment projects, it is always important to take *uncertainty* into account in the best way possible

-   To that matter, an important question that remains is: **how to ensure that the NPV you're measuring is really [positive]{.green}?**

-   In this lecture, we will look at some ares in which an investment analyst must consider in order to make reasonable investment decisions:
...
-   Suppose you have your project sponsors to give you [honest]{.blue} forecasts. These forecasts are considered *unbiased*: in other words, on average, the error will be *zero*
...
-   A [competitive advantage]{.blue} will protect a firm only if it is [durable]{.blue} and can be [sustained]{.blue} against competition from other businesses!
...
Assets=c('GOOG','AMZN','NVDA','MSFT')

Data=do.call('merge.xts',lapply(Assets,getSymbols,auto.assign=FALSE)%>%map(xts::.subset.xts,j=6)%>%map(dailyReturn)%>%map(~cumprod(1+.x)))%>%
  setNames(Assets)
...
highchart(type = "stock") %>% 
  hc_title(text="Price trends for tech stocks")%>%
  hc_subtitle(text='Considering closing adjusted prices from Yahoo! Finance.')%>%
  hc_add_series(Data[,1],name=Assets[1],type='line')%>% 
  hc_add_series(Data[,2],name=Assets[2],type='line')%>% 
  hc_add_series(Data[,3],name=Assets[3],type='line')%>% 
  hc_add_series(Data[,4],name=Assets[4],type='line')%>% 
...
-   [Positive]{.green} NPV projects should be accompanied with [clear competitive advantage rationales]{.blue}:
...
- At the end of the day, it is much more about the *assumptions* about future cash flows
