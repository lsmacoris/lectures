...
[^1]: Note that we're using *Net Debt* $\small (320-20)$ to weight-in the debt potion in the capital structure
...
![](Figures/F6.png){fig-align='center' width=50%}
...
-   Why we should [unlever]{.blue} the return on equity^[A similar rationale can be applied to the equity $\small \beta$ - See [@BDM] and the *Appendix* for a detailed discussion.]? Note that whenever we measure the cost of equity, we are measuring the expected returns based on the [equity]{.blue} risk, which has some implications if a given firm has debt:
