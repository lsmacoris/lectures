...
format:
  revealjs:
    title: '5. Short-term Financial Planning'
...
WC=read_excel('Exercises/Financials.xlsx',sheet=9)[4:8,]%>%
...
Sales=read_excel('Exercises/Financials.xlsx',sheet=3)[1,]%>%
...
Data=read_excel('Exercises/Financials.xlsx',sheet=9)[4:8,]%>%
...
Data=read_excel('Exercises/Financials.xlsx',sheet=5)%>%
  gather('Key','Value',-Account)%>%
...
![](Figures/F1.jpg){height="750" fig-align="center"}

## Step 2: Financing Short-term Financing Needs

-   Right after forecasting the need for short-term financing, it is time to decide [how]{.blue} it will be financed
...
-   The [Matching Principle]{.blue}: firms can increase their value by adopting a policy that minimizes these kinds of costs:

1.  [Short-term needs]{.blue} should be financed with [Short-term debt]{.blue}

2.  [Long-term needs]{.blue} should be financed with [Long-term sources of funds]{.blue}

## Matching Principle in practice - Permanent Working Capital

-   We saw that we can distinguish permanent and short-term working capital needs.
...
-   The matching principle indicates that the firm should finance this [permanent]{.blue} portion of working capital with [long-term]{.blue} sources of funds
...
**Example:** after forecasting permanent working capital needs, if funding occurs through short-term sources (say, 1 year), firms are exposed to [interest rate risk]{.blue} -- you may have to refinance at a higher rate in the future! During *COVID-19*, interest rate risk made smaller firms to shut-down its operations.
...
-   This [temporary working capital]{.blue} need is the difference between the actual level of investment in short-term assets and the permanent working capital investment. Following the Matching Principle, it should be financed with short-term sources!
...
-   Download *Arezzos*'s Annual Financial Statements through its *Investors Relations* website (access [here](https://ri.arezzoco.com.br/en/), also available on eClass®)
...
(+) [Interest on borrowed funds]{.blue}: \$800,000 $\times$ 10% = 80,000\
(+) [Commitment on unused portion]{.blue}: \$200,000 $\times$ 0.5% = 1,000\
(=) [Total Cost]{.blue} = 81,000\
(=) [Effective Interest Rate, inclusive of Fees]{.blue}: (881,000/800,000)-1=**10.125%**
...
1.  The amount of the loan origination fee is $1\% \times 500,000 = 5,000$
2.  The actual amount borrowed is $500,000-5,000=495,000$\
3.  The interest rate is charged on the total, not the discounted value: \$500,000 $\times$ (3%) = $15,000$
4.  Therefore, the annual effective interest rate is 515,000/495,000 - 1 = 4.04%
...
1.  If that is the case, then the requirement amount is 10% $\times$ 500,000 = 50,000
2.  Thus, the firm has only \$450,000 of the loan proceeds actually available for use, although it must pay interest on the full loan amount
...
## Commercial Papers in the U.S. at the onset of the COVID-19 pandemic:
...
2.  The bank then extends a loan to the firm, that will receive a *discounted* value at the present time
3.  The bank will then receive the original amount in the *accounts receivable*
...
## Secured Financing in Brazil - *Vendor*

-   A [vendor]{.blue} operation occurs when a firm sells a product to a client, which will pay with a predefined date, and [transfers]{.blue} the credit function to the bank

-   The firm bears the responsibility of paying the amount due if the client does not pay

. . .

![](Figures/F3.jpg){fig-align="center" width="800"}

## Secured Financing in Brazil - *Compor*

-   On the other hand, a [compor]{.blue} operation occurs when the customer pays the firm upfront, and the bank provides credit to the customer for a specific commercial operation
...
![](Figures/F4.jpg){fig-align="center" width="800"}
...
-   Similar to a discount operation, the key difference reside in the [risk in the event of non-payment]{.blue}:
...
-   **Key benefit**: *FIDCs* do not need to concentrate risk in only one type of credit operation/customer
