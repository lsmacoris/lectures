...
-   Recall that our cash flows were:
...
NPV_Profile=data.frame(FC=c(-219600,46592,69266,80218,101293,130684),
              t=c(0:5))

NPV=data.frame()

for(r in seq(0.01,0.3,0.01)){
...
Chart=NPV%>%ggplot(aes(x=Discount,y=NPV))+
  geom_line(size=2,col='darkgreen')+
  theme_minimal()+
  labs(x='Discount Rate (r)',y='Net Present Value (NPV)',
       title='')+
  scale_y_continuous(labels=scales::dollar)+
  scale_x_continuous(labels=scales::percent,breaks=seq(0.01,0.3,0.05))+
  geom_hline(yintercept = 0,linetype='dashed')+
  geom_vline(xintercept = 0.226,linetype='dashed')+
  ...
  annotate("text",
           ...
           y=50000,
           xmax=0.03,
           label="For r = 22.6%, the NPV is exactly zero.",size=7)
...
Chart=NPV%>%ggplot(aes(x=Discount,y=NPV))+
  ...
  geom_hline(yintercept = 0,linetype='dashed')+
  geom_vline(xintercept = 0.2338,linetype='dashed')+
  theme(axis.title = element_text(size=20,face='bold'),
        axis.text = element_text(size=15),
        title = element_text(size=25,face='bold'))+
  annotate("text",
           ...
           y=-100000,
           xmax=0.03,
           label="For r = 23.38%, the NPV is exactly zero.",size=7)
...
Chart=NPV%>%ggplot(aes(x=Discount,y=NPV))+
  ...
  annotate("text",
           ...
           y=-25000,
           xmax=0.03,
           label="The are multiple values that satisfy the IRR rule",size=7)
...
Chart=NPV%>%ggplot(aes(x=Discount,y=NPV))+
  ...
  annotate("text",
           ...
           y=150000,
           xmax=0.03,
           label="There is no value that satisfies the IRR rule",size=7)
...
- See [Present Value: A Note on Personal Applications (UV5136)]{.blue} for a detailed 
discussion on how the diffent applications of the *Net Present Value* rule
